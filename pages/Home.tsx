        ...
        <div className="min-h-screen pb-32 bg-[#050505] pt-16">
            ...
            {heroMovie && (
                <div className="relative w-full h-[85vh] md:h-[95vh] min-h-[700px] bg-black overflow-hidden group">
                    ...
                    {/* Content Container */}
                    <div className="absolute inset-0 flex flex-col justify-end md:justify-center p-6 md:p-20 lg:p-32 max-w-[1800px] mx-auto w-full z-10">
                        <div className="max-w-4xl mt-16 md:mt-24">
                            {/* Metadata Badges */}
                            <div className="flex flex-wrap items-center gap-2 md:gap-3 mb-6 animate-slide-right">
                                <span className="bg-blue-600 text-white text-[9px] md:text-xs font-black px-2.5 py-1 rounded-sm uppercase tracking-[0.2em] shadow-[0_0_20px_rgba(37,99,235,0.4)]">
                                    Featured
                                </span>
                                <span className="bg-white/10 backdrop-blur-md text-zinc-300 text-[9px] md:text-xs font-bold px-2.5 py-1 border border-white/10 uppercase tracking-[0.2em]">
                                    {heroMovie.quality || 'FULL HD'}
                                </span>
                                <span className="bg-white/10 backdrop-blur-md text-blue-400 text-[9px] md:text-xs font-bold px-2.5 py-1 border border-white/10 uppercase tracking-[0.2em]">
                                    {heroMovie.year}
                                </span>
                            </div>

                            {/* Main Title with Optimized Typography */}
                            <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-black text-white mb-6 tracking-tighter leading-[1.1] uppercase italic font-premium drop-shadow-[0_10px_30px_rgba(0,0,0,0.8)] animate-slide-right break-words line-clamp-3" style={{ animationDelay: '0.1s' }}>
                            ...
                            <div className="mb-8 animate-slide-right flex items-center gap-4 md:gap-6" style={{ animationDelay: '0.2s' }}>
                                <div className="h-10 md:h-16 w-[2px] bg-gradient-to-b from-blue-600 to-transparent"></div>
                                <div>
                                    <p className="text-zinc-400 text-[10px] md:text-lg font-medium italic opacity-90 leading-tight tracking-wide mb-1 md:mb-2 line-clamp-1">
                                        {heroMovie.origin_name}
                                    </p>
                                    <div className="flex items-center gap-2">
                                        <div className="flex gap-0.5">
                                            {[1, 2, 3, 4, 5].map((s) => (
                                                <Icons.Star key={s} size={10} className="fill-blue-500 text-blue-500" />
                                            ))}
                                        </div>
                                        <span className="text-[8px] md:text-[10px] font-black text-blue-500 uppercase tracking-widest mt-0.5">9.8 RATING</span>
                                    </div>
                                </div>
                            </div>
